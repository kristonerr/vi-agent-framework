...
## 9. Observer — наблюдение и самообучение (опционально)

Observer — безопасный модуль, который следит только за **названием активного окна** (не контент, не скриншоты, не клавиши) и учится через веб-поиск.

### Запуск watcher (слежение за окнами):
```bash
python viagent_observer.py --watch
```

### Разовое обучение (поиск по темам из твоей активности):
```bash
python viagent_observer.py --learn
```

### Всё вместе (watcher + обучение каждый час):
```bash
python viagent_observer.py --watch --learn --daemon
```

Результаты сохраняются в `learnings.md`. Агент будет использовать их в диалогах.
...
- [ ] `python viagent_observer.py --learn` (опционально, самообучение)
